...
public class Tauler extends JPanel {
				...
				if (!acaba) {
					if (!jocPausat) {
							...
							for (int j = 0; j < columnes; j++) {
								...
								if (celdes[i][j].isViva()) {
									...
									if (vecinosVivos < 2 || vecinosVivos > 3) {
										// Muere por soledad o sobrepoblaci�n
						...
						if (generacionCount >= 2 && generacionAnterior != null
							...
							// El juego ha llegado a un estado estacionario, detener la ejecuci�n
							...
							JOptionPane.showMessageDialog(null,
									"El joc ha arribat a un estat estacionari.\nN� Generacions: " + comptadorGeneracions
											+ "\nN� C�l�lules creades: " + celCreades + "\nN� C�l�lules M�xim Arribat: "
											+ max + "\nDensitat M�xima Arribada: " + densitatMaxima * 100 + "%",
							...
							// No hay cambios en la generaci�n, detener la ejecuci�n
							for (int i = 0; i < files; i++) {
								for (int j = 0; j < columnes; j++) {
									celdes[i][j].setViva(nuevaGeneracion[i][j].isViva());
								}
							...
							// Actualizar la generaci�n anterior y el contador
	...
	private static int contarVecinosVivos(int fila, int columna) {
		...
		// Restar la propia celda si est� viva
		if (celdes[fila][columna].isViva()) {
			contador--;
		}
